    ...
    void InitializePlayerUI()
    {
        for (int x = 0; x < playerContainers.Length; ++x)
        {
            ...
            if (x < PhotonNetwork.PlayerList.Length)
            {
                ...
                container.hatTimeSlider.maxValue = GameManager.instance.timeToDie;
            }
            else
    ...
    void UpdatePlayerUI()
    {
        for (int x = 0; x < GameManager.instance.players.Length; ++x)
        {
            if (GameManager.instance.players[x] != null)
                playerContainers[x].hatTimeSlider.value = GameManager.instance.timeToDie - GameManager.instance.players[x].curHatTime;
        }
    }
